...
import { insert as insertItem } from '../../model/feed-items.mjs';
import { processFeedItems } from '../../actions/index.mjs';
...
const pageFetchReport = async (req, res) => {
  ...
  logger.info(`${MODULE_NAME} 3D7D4AA0: Inserting items`, { traceId });
  data.items?.map(async (item) => {
    await insertItem(traceId, url, item);
    return null;
  });

  await processFeedItems(traceId, url, data.items);
};
